...
public class CuotaDaoImp implements CuotaDao {
	...
	private static final String sumarCuotasPendientes = "SELECT p.importe_cuota * COUNT(c.id_cuota) as suma FROM prestamos p JOIN cuotas c ON p.id_prestamo = c.id_prestamo "
			+ "WHERE p.id_prestamo = ? AND c.estado_pago = 0;";
